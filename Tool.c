...
//打印Diaond文件格式，文件名为“Diamond.txt”(三合金)
void printDiamond(int *note,double *x,double *y,double *z,ATOM *atoms,int atomsTypeCount,int N,char *path){
	FILE *fp;
	int i;
	fp = fopen(path,"w");
	fprintf(fp,"%d\n",N);
	for(i = 0; i < atomsTypeCount; i++)
		fprintf(fp,"%s",GetAtomPara(atoms[i]).name);
	fprintf(fp,",NCS\n");
	for(i=0;i<N;i++){
		fprintf(fp,"%s\t%lf\t%lf\t%lf\n",GetAtomPara(atoms[note[i]]).name,x[i],y[i],z[i]);
	}
	fclose(fp);
}
...
void SD_File(char *input, ATOM *atoms, int atomTypeCount, int N)
{
	...
	Distance(x,y,z,R,N);
	printf("a=%lf\n",Rmin*sqrt(2));
	printDiamond(note,x,y,z,atoms,atomTypeCount,N,input);
